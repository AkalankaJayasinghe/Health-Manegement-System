...
$queryu = "SELECT * FROM patients WHERE pid = :user_id"; // Use 'id' in 'patients' table
...
$doctor_query = "SELECT did, name FROM doctors"; // Use 'doctors' table
$stmtd = $conn->prepare($doctor_query);
$stmtd->execute();
$doctors = $stmtd->fetchAll(PDO::FETCH_ASSOC);

// Fetch appointment details from the 'appointments' table
$querya = "SELECT * FROM appointments WHERE aid = :aid"; // Use parameterized query
$stmta = $conn->prepare($querya);
$stmta->bindParam(':aid', $_GET['aid']);
$stmta->execute();
$datas = $stmta->fetchAll(PDO::FETCH_ASSOC);
$id = $datas[0]['did']; // Doctor ID

// Fetch the doctor's details from the 'doctors' table
$queryn = "SELECT * FROM doctors WHERE did = :doctor_id"; // Use 'doctors' table
$stmtn = $conn->prepare($queryn);
$stmtn->bindParam(':doctor_id', $id);
$stmtn->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_appointment'])) {
    ...
    $update_query = "UPDATE appointments 
                     SET did = :doctor_id, appointment_date = :appointment_date, appointment_time = :appointment_time 
                     WHERE aid = :aid";
    $stmtup = $conn->prepare($update_query);
    $stmtup->bindParam(':doctor_id', $doctor_id);
    $stmtup->bindParam(':appointment_date', $appointment_date);
    $stmtup->bindParam(':appointment_time', $appointment_time);
    $stmtup->bindParam(':aid', $aid);

    if ($stmtup->execute()) {
        $appointment_success = "Appointment successfully updated!";
    }
}
    ...
    <style>
        ...
        * {
        ...
        th {
            ...
            position: sticky;
            top: 0;
            /* Makes table header fixed when scrolling */
            background: white;
            z-index: 2;
        }
...
<body>
    ...
    <div class="main-content">
        ...
        <h3 style="margin-bottom: 40px;"> Update an Appointment </h3>
                    ...
                    <div class="appointment-form">
                        <div class="form-group">
                            ...
                            <select id="doctor_names" name="doctor_id" required>
                                <option value="<?php echo $data['did']; ?>"><?php echo ucfirst($datan['name']); ?></option>
                                <?php foreach ($doctors as $doctor): ?>
                                    <option value="<?php echo htmlspecialchars($doctor['did']); ?>">
                                        <!-- Change from uid to did -->
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" id="appointment_date" name="appointment_date"
                                value="<?php echo $data['appointment_date']; ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="appointment_time">Appointment Time</label>
                            <input type="time" id="appointment_time" name="appointment_time"
                                value="<?php echo $data['appointment_time']; ?>" required>
                            <span id="timeError" class="error-message" style="width: 200px;"></span>
                        </div>
                    <?php endforeach; ?>

                    <div class="form-group">
                        <button type="submit" name="update_appointment" style="margin:20px 10px">Update
                            Appointment</button>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        document.getElementById("appointment_time").addEventListener("change", function () {
            ...
            // Auto-adjust to the nearest 30-minute interval
        ...
        // Set the minimum selectable time dynamically
